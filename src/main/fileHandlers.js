...
// Handle the temporary directory
const tempDir = path.join(os.tmpdir(), "nanoglobin");

// Function to parse an NGB file
export async function parseNGBFile(filePath, fileBuffer) {
  ...
  for (const barcode of folders) {
    const haploPath = Object.keys(zip.files).find((filePath) => filePath.startsWith(`${barcode}/`) && filePath.endsWith("haplo.csv"));
    ...
    const lengthPath = Object.keys(zip.files).find((filePath) => filePath.startsWith(`${barcode}/`) && filePath.includes("length.csv"));
    ...
    barcodes[barcode] = {
      bam: Object.keys(zip.files).find((filePath) => filePath.startsWith(`${barcode}/`) && filePath.endsWith(".bam")) || null,
      bai: Object.keys(zip.files).find((filePath) => filePath.startsWith(`${barcode}/`) && filePath.endsWith(".bai")) || null,
      vcf: Object.keys(zip.files).find((filePath) => filePath.startsWith(`${barcode}/`) && filePath.endsWith(".vcf")) || null,
...
// Function to extract BAM and BAI
export async function extractBAMAndBAIFromBuffer(fileBuffer, barcode) {
  const zip = await JSZip.loadAsync(fileBuffer);
  const bamPath = Object.keys(zip.files).find((filePath) => filePath.startsWith(`${barcode}/`) && filePath.endsWith(".bam"));
  const baiPath = Object.keys(zip.files).find((filePath) => filePath.startsWith(`${barcode}/`) && filePath.endsWith(".bai"));
